class Info{
    constructor(page){
        ...
        this.vendorLastName = '#rc_select_24'
        ...
        this.discard = "//button[normalize-space()='Discard Form']"
        this.aadharNumber = "//input[@placeholder='Aadhaar Number']"
        this.aadharUpload = "//div[4]//div[2]//div[1]//span[1]//div[1]//span[1]//*[name()='svg']"
    ...
    async newVendorWithPAN(panNumber, venSuggName, activeFrom) {
        ...
        await this.page.waitForTimeout(500); // Optional: wait for a short period to allow error messages to update

        // Check if an error exists right after entering the PAN number
        const errorExists = await this.page.locator("//span[@class='error']").isVisible();
        if (errorExists) {
            const errorMessage = await this.page.locator("//span[@class='error']").textContent();
            console.error(`Error encountered with PAN number: ${panNumber}, Message: ${errorMessage}`);
            
            // Check for specific error messages
            if (errorMessage.includes('Vendor already exists') || errorMessage.includes('Invalid PAN Number')) {
                console.log(`Vendor already exists or Invalid PAN for PAN: ${panNumber}. Discarding form and closing the browser...`);
                await this.page.locator('button:has-text("Discard")').click(); // Assuming there's a discard button
                
                // Print a message indicating the test is terminated
                console.log(`Test terminated for PAN: ${panNumber}.`); 
                await this.closeBrowser(); // Close the browser after handling the error
                return; // Immediately return to stop any further processing
            } else {
                console.error("Unknown error occurred, stopping execution.");
                console.log("Test terminated due to an unknown error."); 
                await this.closeBrowser(); // Close the browser after handling the error
                return; // Immediately return to stop any further processing
            }
        }

        // If no error, continue with the vendor creation process
        ...
        console.log(`Vendor created successfully for PAN: ${panNumber}.`);
        return true; // Return true to indicate success
    }

    async closeBrowser() {
        console.log("Closing the browser...");
        await this.page.context().close(); // Close the current page context
    ...
    async Credit(creditLimit) {
        if (creditLimit.toString().length > 10) {
            ...
            creditLimit = creditLimit.toString().slice(0, 10);
        }
    ...
    async taxApproval() {
        await this.page.route("**/v1/mdm/business-partner/reject", async (route) => {
            const response = await route.fetch();
            const message = await response.json();
            console.log("API Response after rejection:", JSON.stringify(message, null, 2));
            route.continue();
        });
        await this.page.locator(this.vendorFirstName).click();
        await this.page.locator(this.vendorNameDropdown).first().click();
        await this.page.locator(this.taxReject).click();
        await this.page.locator(this.rejectReason).fill("Please change Business category.....&& Please Provide Aadhar Number in additional Information!!");
        await this.page.locator(this.rejectBtn).click();
        await this.page.waitForResponse("**/v1/mdm/business-partner/reject");
    
        console.log("Vendor rejected. Proceeding to reopen the vendor record for resubmission.");
    }
    
    async reopenRejectedVendor() {
        console.log("Reopening the rejected vendor record...");
    
        await this.paginateAndClickLastRecord();
        
        console.log("Rejected vendor record reopened. User can now resubmit the request.");
    }
    async resubmitBusinessCategory(businessCategory){
            await this.page.locator(this.businessCategory).click();
            const optionToSelect = this.businessCategoryOption(businessCategory);
            await this.page.locator(optionToSelect).click();
            await this.page.locator(this.saveNext).click(); 
    ...
    async resubmitGstInfoReg(){
        await this.page.locator(this.saveContinue).click(); 
    }
    async resubmitBankInfo(){
        await this.page.locator(this.saveContinue).click(); 
    }
    async resubmitAddressInfo(){
        await this.page.locator(this.saveContinue).click();
    }
    async resubmitAdditionalInfo(){
        // await this.page.locator(this.aadharNumber).fill(aadharNumber);
        // await this.page.locator(this.aadharUpload).setInputFiles('tests/files/fcbfb557-965b-4cc4-95b2-a044c21d761e.pdf'); 
        // await this.page.waitForTimeout(2000)
        await this.page.locator(this.submit).click();

    }
    async resubmitRequest(){
        await this.resubmitPOC()
        await this.resubmitGstInfoReg()
        await this.resubmitBankInfo()
        await this.resubmitAddressInfo()
        await this.resubmitAdditionalInfo('779923237687')
        await this.requestOnboard()
    }
    async approveVendorAtTaxLevel() {

            await this.page.route("**/v1/mdm/business-partner/update", async (route) => {
            const response = await route.fetch();
            const message = await response.json();
            console.log("API Response after approval:", JSON.stringify(message, null, 2));
            route.continue();
        });
        await this.page.locator(this.vendorLastName).click();
        await this.page.locator(this.vendorNameDropdown).nth(1).click();
